(function () {
    ...
    function setExportFeedback(message = "", type = "") {
        const note = document.querySelector(".season-summary-book [data-season-summary-export-note]");
        if (!note) return;
        note.className = `season-summary-closing-export-note${type ? ` ${type}` : ""}`;
    ...
    function waitForNextFrame(count = 2) {
        return new Promise((resolve) => {
            ...
            step(Math.max(1, Number(count || 0)));
        });
    }
    ...
    function buildExportFileName() {
        const teamName = String(state.profile?.teamName || state.profile?.cover?.display_name || "").trim();
        const safeTeamName = teamName
            .replace(/[^\w\u4e00-\u9fa5-]+/g, "-")
            .replace(/-+/g, "-")
            .replace(/^-|-$/g, "");
        const safeUid = String(state.lastUid || "season-summary")
            .replace(/[^\w-]+/g, "-")
            .replace(/-+/g, "-")
            .replace(/^-|-$/g, "");
                ...
                <div class="season-summary-story-main">
                    <div class="season-summary-story-shell season-summary-closing-copy">
                        <div class="season-summary-story-copy">
                            ...
                            <p class="season-summary-story-paragraph">是享受与朋友们嬉笑怒骂的瞬间，还是对接下来的长草期感到枯燥和无聊，又或者是转为对季后赛的期待</p>
                            <p class="season-summary-story-paragraph">不管怎样，感谢你一个赛季的坚持，让这些冰冷的数据都显得有意义了起来，期待我们在下一个时刻的相遇</p>
                            <div class="season-summary-closing-artwork">
                                <img
                                    class="season-summary-closing-artwork-image"
                                    src="./assets/raynaud_01.png"
                                    alt="Raynaud closing artwork"
                                    loading="lazy"
                                    decoding="async"
                                />
                            </div>
    ...
    window.SeasonSummaryPage = {
        load: loadSummary,
        goToPage,
        exportImage: exportSeasonSummaryImage,
    };
}());
