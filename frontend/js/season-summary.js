(function () {
    ...
    function renderTransferStoryV2(profile) {
        ...
        let paragraphThree = "你在不同比赛日的出手节奏比较平均，很难说自己永远只相信某一个特定的时机。";
        if (favoriteDayNumber >= 1 && favoriteDayNumber <= 3) {
            paragraphThree = `${mark(`Day${favoriteDayNumber}`)}也是你最常出手的日子，拿到 FT 就该趁早用。`;
        } else if (favoriteDayNumber >= 4 && favoriteDayNumber <= 7) {
            paragraphThree = `经常把转会留到${mark(`Day${favoriteDayNumber}`)}再出手，不仅规划得当，而且沉得住气。`;
        }

        let paragraphFour = "至于换人的钟点，你更像是顺着消息和感觉走，不会被单一节奏彻底绑住。";
        if (Number.isFinite(startHour)) {
            if (startHour >= 8 && startHour < 20) paragraphFour = `你好像更喜欢在${mark(timeSlotLabel)}换人，伤病报告都是小事，心情＞fantasy。`;
            if (startHour >= 20 && startHour < 24) paragraphFour = `你好像更喜欢在${mark(timeSlotLabel)}换人，谨慎而大胆的选择，等到消息更完整再操作，也不耽误睡觉时间。`;
            if (startHour >= 0 && startHour < 6) paragraphFour = `你好像更喜欢在${mark(timeSlotLabel)}换人，夜生活才是你的舞台，必须看到想换的球员 available 才放心换人睡觉。`;
            if (startHour >= 6 && startHour < 8) paragraphFour = `你好像更喜欢在${mark(timeSlotLabel)}换人，定闹钟早起刷消息，伍哥别装了，我知道是你~`;
        }
